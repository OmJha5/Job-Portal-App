...
export default function LatestJobCards({ job }) {
  return (
    <div className='p-10 rounded-md shadow-xl border border-gray-100 cursor-pointer'>
        <div>
            <h1 className='text-3xl font-medium'>{job?.companyId?.name}</h1> 
            <p className='text-sm my-2 text-gray-500'>{job.location}</p>
        </div>

        <div className='my-2'>
            <h1 className='text-2xl font-normal mb-2'>{job.title}</h1>
